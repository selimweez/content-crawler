...
// Test URL
$menuUrl = 'https://notifybee.com.tr/menu?id=1584';
...
echo "<h1>Düzeltilmiş Alt Kategori Keşfi Test</h1>\n";
echo "<h2>Menu URL: $menuUrl</h2>\n";

// 1. Gelişmiş kategori keşfi (alt kategoriler dahil)
echo "<hr><h3>1. Gelişmiş Kategori Keşfi (Alt Kategoriler Dahil)</h3>\n";
...
if ($categoryResult['success']) {
    ...
    echo "<p><strong>Alt kategoriler dahil:</strong> " . ($categoryResult['includes_subcategories'] ? 'Evet' : 'Hayır') . "</p>\n";
    
    echo "<h4>Bulunan Kategoriler:</h4>\n";
    echo "<table border='1' style='border-collapse: collapse; width: 100%;'>\n";
    echo "<tr><th>No</th><th>Kategori Adı</th><th>URL</th><th>Tip</th></tr>\n";
    
    foreach ($categoryResult['categories'] as $index => $category) {
        $isMainCategory = $category['is_main_category'] ?? true;
        $categoryType = $isMainCategory ? 'Ana Kategori' : 'Alt Kategori';
        $categoryStyle = $isMainCategory ? '' : 'background-color: #f0f8ff;';
...
if ($crawlResult['success']) {
    ...
    foreach ($crawlResult['crawl_results'] as $result) {
        ...
        $isMainCategory = isset($result['is_main_category']) && $result['is_main_category'];
        $preDiscovered = isset($result['pre_discovered']) && $result['pre_discovered'];
        
        if ($isSubcategory) {
            $categoryType = 'Alt Kategori';
            $categoryStyle = 'background-color: #f0f8ff;';
        } elseif ($isMainCategory) {
            $categoryType = 'Ana Kategori';
            $categoryStyle = '';
        } else {
            $categoryType = 'Bilinmeyen';
            $categoryStyle = 'background-color: #fff0f0;';
        }
        
        if ($preDiscovered) {
            $categoryType .= ' (Önceden Keşfedilmiş)';
        }
